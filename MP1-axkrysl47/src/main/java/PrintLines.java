...
public class PrintLines {
     ...
     * @param actress       actor/actress to print all lines for
     * @param scriptLines   script to print lines from
     */
    public static void printLinesFor(final String actress, final String[] scriptLines) {
        String target = actress.toUpperCase();

        for (int i = 0; i < scriptLines.length; i++) {
            if (scriptLines[i].equals(target) == true) {
                System.out.println(actress.toUpperCase());
                break;
            }
            if (i >= scriptLines.length - 1 && scriptLines[i].equals(target) == false) {
                return;
            }
        }
        ...
        for (int i = 0; i < scriptLines.length; i++) {
            if (scriptLines[i].equals(target) == true) {
                System.out.println("---");
                while (i + 1 < scriptLines.length && scriptLines[i + 1].compareTo("") != 0) {
                    i++;
