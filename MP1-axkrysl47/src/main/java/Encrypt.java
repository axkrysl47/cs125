...
public class Encrypt {
     ...
     *
     * A function that encrypts a given array of characters using an rotate-N transformation.
     *
     * @param line  array of characters to encrypt
     * @param shift integer to use in rotate-N transformation
     *
     * @return      encrypted array of characters
     */
    public static char[] encrypt(final char[] line, final int shift) {
        ...
        for (int i = 0; i < num.length; i++) {
            num[i] = num[i] + shift;
            while (num[i] < TRANSFORM_START) {
                num[i] += TRANSFORM_MODULUS;
            }
            while (num[i] > TRANSFORM_END) {
                num[i] -= TRANSFORM_MODULUS;
            }
        }
